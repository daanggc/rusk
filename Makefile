...
run-dev: ## Launch a local ephemeral node for development
	...
	DUSK_CONSENSUS_KEYS_PASS=password cargo r --release --no-default-features --features default-without-archive -p rusk -- -s /tmp/example.state || \
...
run-dev-archive: ## Launch a local ephemeral archive node for development
	...
	DUSK_CONSENSUS_KEYS_PASS=password cargo r --release --features default -p rusk  -- -s /tmp/example.state || \
	echo "Failed to start the node. Make sure you have run 'make prepare-dev' before running this command"
